...
//		List<String> attempMsapPathCuidList = new ArrayList<String>();
	...
	public void releaseAttempTraphTransRelation(ServiceActionContext ac, List<String> attempTransPathCuidList, List<String> attempTraphCuidList) {
		...
		if(tempTransPathCuidList != null && !tempTransPathCuidList.isEmpty()) {
			...
			if(attempTransPtpCuidList != null && !attempTransPtpCuidList.isEmpty()) {
				...
				this.setPtpState(attempTransPtpCuidList, 1);
			}
	...
	public void releaseAttempTraphPtnRelation(ServiceActionContext ac, List<String> attempPtnPathCuidList, List<String> attempTraphCuidList) {
		if(attempPtnPathCuidList != null && !attempPtnPathCuidList.isEmpty()) {
			...
			if(attempPtnPtpCuidList != null && !attempPtnPtpCuidList.isEmpty()) {
				// 设置端口状态为空闲
				logger.info("设置端口状态为空闲");
				this.setPtpState(attempPtnPtpCuidList, 1);
			}
	...
	*/
	 ...
	 * @param state
	 */
	public void setPtpState(List<String> ptpCuidList,int state){
...
//		pm.put("oldState", oldState);
	 ...
	 * @param state
	 */
	public void setPtpNaState(List<String> ptpCuidList,int state){
...
//						ctpRateMap.put(cuid, logicRate);
	...
	public void copyBslteByCuid(Map<String,Object> addMesMap, Map routeMap, Map nmlteMap, String taskCuid){
		...
		String routeDescription = IbatisDAOHelper.getStringValue(routeMap, "ROUTE_DESCIPTION");
...
//		String aPortCuid = IbatisDAOHelper.getStringValue(routeMap, "L2L3_PORT_CUID");
		String aPortCuid = IbatisDAOHelper.getStringValue(routeMap, "RELATED_A_PTP_CUID");
		
		 String apptempTraphCuid = IbatisDAOHelper.getStringValue(routeMap, "RELATED_ROUTE_CUID");
		    List<Map> AZ_ROOM_CUID = new ArrayList();
		    Map mp = new HashMap();
		    mp.put("apptempTraphCuid", apptempTraphCuid);

		    String RELATED_A_ROOM_CUID = "";
		    String RELATED_Z_ROOM_CUID = "";
		     AZ_ROOM_CUID = this.IbatisResDAO.getSqlMapClientTemplate().queryForList("TraphMaintain.getAZROOMCuidByCuids", mp);
		    if (AZ_ROOM_CUID.size() > 0) {
		      for (Map AZCuidMap : AZ_ROOM_CUID) {
		        RELATED_A_ROOM_CUID = IbatisDAOHelper.getStringValue(AZCuidMap, "RELATED_A_ROOM_CUID");
		        RELATED_Z_ROOM_CUID = IbatisDAOHelper.getStringValue(AZCuidMap, "RELATED_Z_ROOM_CUID");
		      }
		    }

		    if ((RELATED_A_ROOM_CUID.length() > 0) && (RELATED_Z_ROOM_CUID.length() > 0)) {
		      attempTraphRecord.addColValue("RELATED_A_ROOM_CUID", RELATED_A_ROOM_CUID);
		      attempTraphRecord.addColValue("RELATED_Z_ROOM_CUID", RELATED_Z_ROOM_CUID);
		    }
			...
			for(String key : newDataMap.keySet()) {
				...
				if(obj instanceof String) {
				 ...
				 if ("END_SWITCH_DEV_A".equals(key)) {
			            ...
			            {
			              rd.addColSqlValue("END_SWITCH_DEV_A", "SELECT LABEL_CN FROM SWITCH_ELEMENT WHERE CUID='" + value + "'");
			            ...
			            if (value.toString().startsWith("SWITCH_ELEMENT")) {
			              rd.addColSqlValue("END_SWITCH_DEV_Z", "SELECT LABEL_CN FROM SWITCH_ELEMENT WHERE CUID='" + value + "'");
			            ...
			            if (value.toString().startsWith("SWITCH_PORT")) {
			              rd.addColSqlValue("END_SWITCHDEV_PORT_A", "SELECT LABEL_CN FROM SWITCH_PORT WHERE CUID='" + value + "'");
			            ...
			            if (value.toString().startsWith("SWITCH_PORT")) {
			              rd.addColSqlValue("END_SWITCHDEV_PORT_Z", "SELECT LABEL_CN FROM SWITCH_PORT WHERE CUID='" + value + "'");
			...
			if(StringUtils.isNotBlank(newAttTraph.getExtIds()) && !newAttTraph.getExtIds().equals(oldAttTraph.getExtIds())) {
				...
				if(nRd == null) {
					...
					TraphName nName = nNameList.get(0);
					 String noStr = "00000" + oldAttTraph.getNo();
			         noStr = noStr.substring(noStr.length() - 4);
			         String labelCn = nName.getaStation() + "-" + nName.getzStation() + nName.getRateName() + noStr;
			         labelCn = labelCn + this.TraphNameBO.getServiceName(newAttTraph.getExtIds());
		            rd.addColValue("LABEL_CN", labelCn);
				}
...
//					r.addColValue("ALIAS", map.get("ALIAS")==null?service.getAlias():map.get("ALIAS"));//别名
					r.addColValue("ALIAS", map.get("ALIAS") == null ? service.getDataMap().get("ALIAS") : map.get("ALIAS"));
				...
				if ((map.get("END_SWITCH_DEV_A") != null) && (map.get("END_SWITCH_DEV_A").toString().length() > 0)) {
			          if (map.get("END_SWITCH_DEV_A").toString().startsWith("TRANS_ELEMENT")) {
			            r.addColSqlValue("END_SWITCH_DEV_A", "SELECT LABEL_CN FROM TRANS_ELEMENT WHERE CUID='" + map.get("END_SWITCH_DEV_A") + "'");
			            r.addColValue("RELATED_A_END_SWITCHDEV_CUID", map.get("END_SWITCH_DEV_A"));
			          } else {
			            r.addColValue("END_SWITCH_DEV_A", map.get("END_SWITCH_DEV_A"));
			          }
			        }
			        if ((map.get("END_SWITCH_DEV_Z") != null) && (map.get("END_SWITCH_DEV_Z").toString().length() > 0)) {
			          if (map.get("END_SWITCH_DEV_Z").toString().startsWith("TRANS_ELEMENT")) {
			            r.addColSqlValue("END_SWITCH_DEV_Z", "SELECT LABEL_CN FROM TRANS_ELEMENT WHERE CUID='" + map.get("END_SWITCH_DEV_Z") + "'");
			            r.addColValue("RELATED_Z_END_SWITCHDEV_CUID", map.get("END_SWITCH_DEV_Z"));
			          } else {
			            r.addColValue("END_SWITCH_DEV_Z", map.get("END_SWITCH_DEV_Z"));
			          }
			        }
			        if ((map.get("END_SWITCHDEV_PORT_A") != null) && (map.get("END_SWITCHDEV_PORT_A").toString().length() > 0)) {
			          if (map.get("END_SWITCHDEV_PORT_A").toString().startsWith("PTP")) {
			            r.addColSqlValue("END_SWITCHDEV_PORT_A", "SELECT LABEL_CN FROM PTP WHERE CUID='" + map.get("END_SWITCHDEV_PORT_A") + "'");
			            r.addColValue("RELATED_A_END_SWITCH_PORT_CUID", map.get("END_SWITCHDEV_PORT_A"));
			          } else {
			            r.addColValue("END_SWITCHDEV_PORT_A", map.get("END_SWITCHDEV_PORT_A"));
			          }
			        }
			        if ((map.get("END_SWITCHDEV_PORT_Z") != null) && (map.get("END_SWITCHDEV_PORT_Z").toString().length() > 0)) {
			          if (map.get("END_SWITCHDEV_PORT_Z").toString().startsWith("PTP")) {
			            r.addColSqlValue("END_SWITCHDEV_PORT_Z", "SELECT LABEL_CN FROM PTP WHERE CUID='" + map.get("END_SWITCHDEV_PORT_Z") + "'");
			            r.addColValue("RELATED_Z_END_SWITCH_PORT_CUID", map.get("END_SWITCHDEV_PORT_Z"));
			          } else {
			            r.addColValue("END_SWITCHDEV_PORT_Z", map.get("END_SWITCHDEV_PORT_Z"));
			          }
			        }
			        r.addColValue("END_SWITCH_DF_PORT_A", map.get("END_SWITCH_DF_PORT_A"));
			        r.addColValue("END_SWITCH_DF_PORT_Z", map.get("END_SWITCH_DF_PORT_Z"));
			        r.addColValue("JHROOM_ACUID", map.get("JHROOM_ACUID"));
			        r.addColValue("JHROOM_ZCUID", map.get("JHROOM_ZCUID"));
			        r.addColValue("JHROOM_A", map.get("JHROOM_A"));
			        r.addColValue("JHROOM_Z", map.get("JHROOM_Z"));
			        r.addColValue("JHSITE_A", map.get("JHSITE_A"));
			        r.addColValue("JHSITE_Z", map.get("JHSITE_Z"));
			        r.addColValue("END_VLANID_A", map.get("END_VLANID_A"));
			        r.addColValue("END_VLANID_Z", map.get("END_VLANID_Z"));
			        r.addColValue("ADDRESS_A", map.get("ADDRESS_A"));
			        r.addColValue("ADDRESS_Z", map.get("ADDRESS_Z"));
			        r.addColValue("IMPORT_LEVEL", map.get("IMPORT_LEVEL"));
			        r.addColValue("NOTES", map.get("NOTES") == null ? service.getDataMap().get("NOTES") : map.get("NOTES"));

			        r.addColValue("RELATED_USER_CUID", map.get("RELATED_USER_CUID"));
			        r.addColValue("SERVICE_LEVEL", serviceLevel);
			        r.addColValue("OWNERSHIP", ownership);

			        r.addColValue("TRAPH_TYPE", traphType);
			        r.addColValue("PATHINFO", map.get("PATHINFO"));
			        r.addColValue("DESIGN_INFO", map.get("DESIGN_INFO"));

			        if (StringUtils.isNotEmpty(relatedUser)) {
			          r.addColSqlValue("RELATED_USER_CUID", "SELECT MAX(CUID) FROM VP WHERE LABEL_CN='" + relatedUser + "'");
			          r.addColValue("USER_NAME", relatedUser);
			        }
			        ...
			        r.addColValue("CHECK_FLAG", Integer.valueOf(1));
			        r.addColValue("SCHEDULE_STATE", Integer.valueOf(2));
			        r.addColValue("PROTECT_MODE", Integer.valueOf(1));
			        r.addColValue("IS_IMPORTANT_PROTECT", Integer.valueOf(1));
			        r.addColValue("IS_DECLARED", Integer.valueOf(0));
			        r.addColValue("IS_TEST_TRAPH", Integer.valueOf(0));
			        r.addColValue("ISDELETE", Integer.valueOf(0));
			        r.addColValue("INPUT_TYPE", Integer.valueOf(0));
			        r.addColValue("IMPORT_TYPE", Integer.valueOf(0));
...
//			List<String> failPtpList = new ArrayList<String>();
			...
			if ((attempTransPtpCuidList != null) && (!attempTransPtpCuidList.isEmpty())){
		        ...
		        setPtpState(attempTransPtpCuidList, 2);
		      }
			pm.clear();
			pm.put("attempTraphCuidList", attempTraphCuidList);
...
//			}
			if(ptnPtpCuidList != null && !ptnPtpCuidList.isEmpty()) {
				...
				this.setPtpState(ptnPtpCuidList, 1);
			}
			if(attempPtnPtpCuidList != null && !attempPtnPtpCuidList.isEmpty()) {
				// 设置端口状态为占用
				logger.info("设置端口状态为占用");
				this.setPtpState(attempPtnPtpCuidList, 2);
			}
			/*if(failPtpList != null && !failPtpList.isEmpty()) {
				logger.info("施工失败，设置端口状态为空闲");
				this.setPtpState(failPtpList, 1,3);
