 ...
 */
$importjs(ctx+'/commons/utils/FilterChar.js');
...
Ext.extend(Ext.ux.data.DwrProxy, Ext.data.DataProxy, {
	...
	handleResponseException : function(request, response, exception) {		
		...
		error=FilterChar.filterChinese(error);
		var g = error.lastIndexOf('not exactly one row');
		var i = error.lastIndexOf('SQLException:');
		if(g != -1){
			if(Ext){
				Ext.Msg.alert("程序运行异常","子查询多余一行");
			}else{
				alert("子查询多余一行");
			}
			return;
		...
		if(i != -1){
			if(Ext){
				Ext.Msg.alert("程序运行异常", error.substring(i+13));
			}else{
				alert("程序运行异常,请联系管理员:"+error.substring(i+13));
			}
			return;
		}
		if(Ext){
